...
int MAG_robustReadMagModels_1(char* filename, int ptr, int array_size)
{
    char line[MAXLINELENGTH];
    int n, nMax = 0, num_terms, a;
    FILE* MODELFILE;
    ...
    fgets(line, MAXLINELENGTH, MODELFILE);
    if (line[0] == '%')
        MAG_readMagneticModel_SHDF(filename, (MAGtype_MagneticModel * (*)[]) ptr, array_size);
    else if (array_size == 1)
    {
        ...
        num_terms = CALCULATE_NUMTERMS(nMax);
        (*(MAGtype_MagneticModel * (*)[]) ptr)[0] = MAG_AllocateModelMemory(num_terms);
        (*(MAGtype_MagneticModel * (*)[]) ptr)[0]->nMax = nMax;
        (*(MAGtype_MagneticModel * (*)[]) ptr)[0]->nMaxSecVar = nMax;
        MAG_readMagneticModel(filename, (*(MAGtype_MagneticModel * (*)[]) ptr)[0]);
        (*(MAGtype_MagneticModel * (*)[]) ptr)[0]->CoefficientFileEndDate = (*(MAGtype_MagneticModel * (*)[]) ptr)[0]->epoch + 5;
...
int MAG_robustReadMagModels_2(char* filename, MAGtype_MagneticModel* (*magneticmodel), int array_size)
{
    char line[MAXLINELENGTH];
    int n, nMax = 0, num_terms, a;
    FILE* MODELFILE;
