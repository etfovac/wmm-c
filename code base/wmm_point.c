...
int main()
{
	...
	// Error	2	error C4996: 'strncpy': This function or variable may be unsafe. Consider using strncpy_s instead. To disable deprecation, use _CRT_SECURE_NO_WARNINGS. See online help for details.	d:\google drive - shared\programiranje\wmm2015 win - deklinacija\wmm_point.c	48	1	c exe konzola
    ...
    //int aa = MAG_robustReadMagModels_1(filename, (uintptr_t)&MagneticModels, epochs);
    //printf("\n %i \n ", aa); // check, returns 1 for OK

    if (!MAG_robustReadMagModels_1(filename, (uintptr_t)&MagneticModels, epochs)) {
        printf("\n WMM.COF not found.  Press enter to exit... \n ");
        fgets(ans, 20, stdin);
        return 1;
    }
